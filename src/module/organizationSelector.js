...
import { QueryOrganizationAction, QueryDeptOrganizationAction } from '../thunk/organizationThunk';

const OrganizationSelectorPopup = (props) => {
    const dispatch = useDispatch()
    const { value, constraint } = props
    const { onClosePopup, onSubmit } = props
    const { isShow, required, deptOnly } = props
    const { listOrganization } = useSelector(s => s.user)
    ...
    const handleQueryInput = () => {
        ...
        if(deptOnly){
            dispatch(QueryDeptOrganizationAction({ deptID: constraint, input: input }))
        }else{
            dispatch(QueryOrganizationAction({ unitID: constraint, input: input }))
        }
    }
    //選擇
    ...
    useEffect(() => {
        if(isShow){
            if(deptOnly){
                ...
                dispatch(QueryOrganizationAction({ unitID: constraint, input: '' }))
            }
        }
    }, [isShow])
    useEffect(() => {
        if(value){
            dispatch(QueryOrganizationAction({
                                ...
                                {activeTab === 'select' &&
                                    <div className="tab-content">
                                        ...
                                        <table className="table">
                                            ...
                                            <tbody>
                                                {((listOrganization ?? []).slice(0, 100)).map((m, i) =>
                                                    <tr key={i}>
                                                        <td>
...
const OrganizationSelector = (props) => {
    const { value, constraint, onSubmit, placeholder, className, required, disabled, deptOnly } = props
